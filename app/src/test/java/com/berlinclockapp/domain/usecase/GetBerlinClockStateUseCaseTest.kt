...
class GetBerlinClockStateUseCaseTest {
    ...
    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with second light On`() {
        val stringTime = "14:36:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals(LightColor.YELLOW,  berlinClockState.secondLight)
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with top minute lights Off`() {
        val stringTime = "14:04:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals( true,  berlinClockState.topMinuteLights.all { it == LightColor.OFF })
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with at least one top minute light is On`() {
        val stringTime = "14:06:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals( true,  berlinClockState.topMinuteLights.any { it == LightColor.YELLOW })
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with bottom minute lights Off`() {
        val stringTime = "14:05:06"
    ...
    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with top hour lights Off`() {
        val stringTime = "04:05:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals( true,  berlinClockState.topHourLights.all { it == LightColor.OFF })
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with at least one top hour light is On`() {
        val stringTime = "06:06:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals( true,  berlinClockState.topHourLights.any { it == LightColor.RED })
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with bottom hour lights Off`() {
        val stringTime = "05:05:06"

        val berlinClockState = getBerlinClockStateUseCase(stringTime)

        Assert.assertEquals( true,  berlinClockState.bottomHourLights.all { it == LightColor.OFF })
    }

    @Test
    fun `test GetBerlinClockStateUseCase() for a given formatted time and return BerlinClockState with at least one bottom hour light is On`() {
        val stringTime = "06:06:06"
